...
user_id,
f"рџ§ѕ Siz {tariff_name} ({tariff_price}) tarifini tanladingiz.\n\n"
...
welcome_text = (
f"рџ‘‹ Salom, {message.from_user.first_name}! Quiz Pilot Super Mini App tizimiga xush kelibsiz.\n\n"
"рџљЂ Yangi Yangilanish:\n"
"рџ”’ Bizning aqlli to'lov tizimimiz ishga tushdi. Premium rejalarni faollashtirib, cheksiz testlar yarating!\n\n"
"рџ‘‡ Marhamat, pastdagi tugmani bosib ilovani oching!"
)
...
btn_app = telebot.types.KeyboardButton(text="Ilovani ochish рџ“±", web_app=telebot.types.WebAppInfo(url=mini_app_url))
...
err_msg = bot.send_message(message.chat.id, "вќЊ Iltimos, faqat rasm (skrinshot) ko'rinishidagi to'lov chekini yuboring. Qaytadan urinib ko'ring:")
...
btn_approve = telebot.types.InlineKeyboardButton("вњ… Tasdiqlash", callback_data=f"p_app_{tx_id}{user_id}")
btn_reject = telebot.types.InlineKeyboardButton("вќЊ Rad etish", callback_data=f"p_rej{tx_id}_{user_id}")
admin_markup.row(btn_approve, btn_reject)

admin_text = (
f"рџ’° YANGI TO'LOV SO'ROVI!\n\n"
f"рџ‘¤ Foydalanuvchi: {first_name} ({username})\n"
f"рџ†” Telegram ID:{user_id}\n"
f"рџ“¦ Tanlangan Tarif: {tariff_name}\n"
f"рџ’µ To'lov Summasi: {tariff_price}\n"
f"рџ§© Tranzaksiya ID:{tx_id}\n\n"
...
bot.send_message(message.chat.id, "вњ… Rahmat! To'lov chekingiz administratorga yuborildi. Tez orada tekshirilib, tarifingiz faollashtiriladi.")
except Exception as e:
logging.error(f"Admin g'a rasm yuborishda xatolik yuz berdi: {e}")
bot.send_message(message.chat.id, "вљ пёЏ To'lov chekingiz qabul qilindi, biroq adminga bildirishnoma yuborishda muammo bo'ldi. Admin paneldan tekshiriladi.")
...
(f"PRO вњЁ ({tariff_name})", premium_until_timestamp, user_id))
conn.commit()

bot.answer_callback_query(call.id, "To'lov tasdiqlandi!")
try:
bot.edit_message_caption(f"вњ… {call.message.caption}\n\nрџџў TASDIQLANDI!", call.message.chat.id, call.message.message_id)
except Exception:
pass
try:
bot.send_message(user_id, f"рџЋ‰ Tabriklaymiz! Sizning {tariff_name} tarifi uchun qilgan to'lovingiz tasdiqlandi. Ilovada PRO status faollashdi! рџ‘‘")
except Exception:
pass
...
try:
bot.edit_message_caption(f"вќЊ {call.message.caption}\n\nрџ”ґ RAD ETILDI!", call.message.chat.id, call.message.message_id)
except Exception:
pass
try:
bot.send_message(user_id, "вќЊ Siz yuborgan to'lov cheki qabul qilinmadi yoki rad etildi. Agar xatolik bo'lgan deb o'ylasangiz, administratorga murojaat qiling.")
except Exception:
pass
conn.close()
...
conn_check.close()
return {"status": "error", "message": "Sizning bepul 3 ta test yaratish limitingiz tugadi. Iltimos, Premium tarifga o'ting! рџ‘‘"}
conn_check.close()
...
try: bot.send_message(user_id, f"рџ“ќ {final_title[:30]} darsligi bo'yicha jami {len(items)} ta test savoli muvaffaqiyatli tayyorlandi!")
